...
public class FlowRuleChecker {
    ...
    public void releaseFlowToken(Context context) {
        if (context.getCurEntry() == null) {
            return;
        }

        HashMap<Long, Timeout> tokenInfoMap = context.getCurEntry().getTokenInfoMap();
        // if tokenInfoMap is null,the rule is not an cluster concurrent flow control rule.
        if (tokenInfoMap == null) {
            return;
        }
        for (Long tokenId : tokenInfoMap.keySet()) {
            Timeout timeout = tokenInfoMap.get(tokenId);

            /**
             * if timeout is null,the strategy {@link ReSourceTimeoutStrategy} is {@link RuleConstant.DEFAULT_RESOURCE_TIMEOUT_STRATEGY}
             */
            if (timeout == null) {
    ...
    private static TokenResult requestToken(TokenService clusterService, FlowRule rule, Context context, DefaultNode node, int acquireCount, boolean prioritized) {
        ...
        if (grade == RuleConstant.FLOW_GRADE_THREAD) {
            String address = null;
            // the address will make token server know which client request the token.
            // if this is a token client,token server will get the address by netty.
    ...
    private static boolean applyTokenResult(/*@NonNull*/ TokenResult result, FlowRule rule, Context context,
        ...
        switch (result.getStatus()) {
            ...
            case TokenResultStatus.BLOCKED:
                if (rule.getGrade() == RuleConstant.FLOW_GRADE_THREAD && context.getCurEntry().isTryAgain()) {
    ...
    private static void setEntry(final FlowRule rule, final Context context, final long tokenId) {
        ...
        if (entry.getTokenInfoMap() == null) {
            entry.setTokenInfoMap(new HashMap<Long, Timeout>());
        }
        Timeout timeout = null;
        /**
         * set up a task to deal with the resource called timeout.
        */
        if (rule.getClusterConfig().getResourceTimeoutStrategy() != RuleConstant.DEFAULT_RESOURCE_TIMEOUT_STRATEGY) {
            timeout = TimerHolder.getTimer().newTimeout(new TimerTask() {
                ...
                public void run(Timeout timeout) {
                    ReSourceTimeoutStrategy strategy = ReSourceTimeoutStrategyUtil.getTimeoutStrategy();
                    strategy.doWithSourceTimeout(tokenId, timeout);
                }
            }, rule.getClusterConfig().getResourceTimeout(), TimeUnit.MILLISECONDS);
        }
        entry.getTokenInfoMap().put(tokenId, timeout);
    }
}