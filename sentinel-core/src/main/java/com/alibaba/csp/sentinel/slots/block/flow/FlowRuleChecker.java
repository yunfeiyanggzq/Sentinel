...
public class FlowRuleChecker {
    ...
    public boolean canPassCheck(/*@NonNull*/ FlowRule rule, Context context, DefaultNode node, int acquireCount,
        ...
        if (rule.isClusterMode()) {
            prepareForConcurrentBlockStrategy(rule, context, prioritized);
        ...
        if (rule.getClusterConfig().getResourceTimeoutStrategy() != RuleConstant.DEFAULT_RESOURCE_TIMEOUT_STRATEGY) {
            timeout = TimerHolder.getTimer().newTimeout(new TimerTask() {
                ...
                public void run(Timeout timeout) {
                    ReSourceTimeoutStrategyUtil.doResourceTimeoutStrategy(rule.getClusterConfig().getResourceTimeoutStrategy(), tokenId);
                }
    ...
    private static void prepareForConcurrentBlockStrategy(FlowRule rule, Context context, boolean prioritized) {
        if (prioritized && context.getCurEntry() != null && rule.getClusterConfig().getAcquireRefuseStrategy() != RuleConstant.DEFAULT_BLOCK_STRATEGY) {
            context.getCurEntry().setTryAgain(true);
        }
    }
}