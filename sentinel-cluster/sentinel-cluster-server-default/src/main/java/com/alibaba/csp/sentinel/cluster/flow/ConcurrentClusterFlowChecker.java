...
/**
 * In order to realize cluster flow control, we must store the information of each call,
 * find and clear those abnormal calls. When token client acquires a token, it will obtain
 * the tokenId issued by the token server. Token server will store the tokenId and related
 * information in a local cache. When the call ends, the token client will carry the tokenId
 * to the token server to release the resources, and the token server will delete the tokenId
 * corresponding to the call. Through such operations, we can count the nowCalls, that is,
 * the concurrent volume. If the resource is called time out or the token client goes offline,
 * the token server will try to find and delete the corresponding tokenId,
 * so as to obtain the accurate concurrency nowCalls.
 *
