...
public class DefaultClusterTokenClient implements ClusterTokenClient {
    ...
    public TokenResult acquireConcurrentToken(String clientAddress, Long ruleId, int acquireCount, boolean prioritized) {
        ...
        ConcurrentFlowAcquireRequestData data = new ConcurrentFlowAcquireRequestData().setCount(acquireCount)
                .setFlowId(ruleId).setPriority(prioritized);
    ...
    public TokenResult keepConcurrentToken(Long tokenId) {
        ConcurrentFlowReleaseRequestData data = new ConcurrentFlowReleaseRequestData().setTokenId(tokenId);
        ClusterRequest<ConcurrentFlowReleaseRequestData> request = new ClusterRequest<>(ClusterConstants.MSG_TYPE_CONCURRENT_FLOW_KEEP, data);
        try {
            TokenResult result = sendTokenRequest(request);
            logForResult(result);
            return result;
